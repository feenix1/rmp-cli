...
def delete_data(key):
    ...
    with open(DATA_PATH, "r") as f:
        try:
            ...
            print("No existing data found, nothing to delete.")
            return
...
if len(args) <= 1:
    print("Usage: rmp-cli <command> [options]")

if len(args) >= 2 and args[1] == "config":
    if len(args) <= 2:
        print("Usage: rmp-cli config <school name>")
        exit()
...
if len(args) >= 2 and args[1] == "find":
    if len(args) <= 2:
        print("Usage: rmp-cli find <professor name>")
        exit()

    config_school = School.from_dict(load_data("school"))

    if (config_school is None):
        print("No school configured. Please set a school using 'rmp-cli config <school name>'")
        exit()
